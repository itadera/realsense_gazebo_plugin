...
void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->rosnode_ = new ros::NodeHandle(this->rsModel->GetName(), this->GetHandle());

  // initialize camera_info_manager
  this->camera_info_manager_.reset(new camera_info_manager::CameraInfoManager(*this->rosnode_, this->GetHandle()));
  ...
  this->color_pub_ = this->itnode_->advertiseCamera(cameraParamsMap_[COLOR_CAMERA_NAME].topic_name, 2);
  this->ir1_pub_ = this->itnode_->advertiseCamera(cameraParamsMap_[IRED1_CAMERA_NAME].topic_name, 2);
  this->ir2_pub_ = this->itnode_->advertiseCamera(cameraParamsMap_[IRED2_CAMERA_NAME].topic_name, 2);
  this->depth_pub_ = this->itnode_->advertiseCamera(cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name, 2);
  if (pointCloud_) {
    this->pointcloud_pub_ = this->rosnode_->advertise<sensor_msgs::PointCloud2>(pointCloudTopic_, 2, false);
  }
}
...
bool GazeboRosRealsense::FillPointCloudHelper(sensor_msgs::PointCloud2 &point_cloud_msg, uint32_t rows_arg, uint32_t cols_arg, uint32_t step_arg, void *data_arg) {
...
void GazeboRosRealsense::OnNewDepthFrame() {
  ...
  if (pointCloud_ && this->pointcloud_pub_.getNumSubscribers() > 0) {
    ...
    FillPointCloudHelper(this->pointcloud_msg_, this->depthCam->ImageHeight(), this->depthCam->ImageWidth(), 2 * this->depthCam->ImageWidth(), (void *)this->depthCam->DepthData());
